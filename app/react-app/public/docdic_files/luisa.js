    ...
    $(document).ready(
        function(){
           ...
           lang=loadLang(lang); // hay que pasar el lenguaje... esto podría cargar la página o no.
            ...
            // Si la imagen está girada, no se muestran los campos.
            $("#imgGirada").on("click",function(ev){
                // Oculto bloques
                $("#captchaCont").addClass("d-none");
                $("#captchaCont textarea").val("");
            });
            $("#imgNormal").on("click",function(ev){
                // Oculto bloques
                $("#captchaCont").removeClass("d-none");
            });
            $("#presNueva").on("click",function(ev){
                // Oculto bloques
                $(".normalLayout").addClass("d-none");
                $(".layout0").removeClass("d-none");
            });
            $("#presNormal").on("click",function(ev){
                // Oculto bloques
                $(".normalLayout").removeClass("d-none");
                $(".layout0").addClass("d-none");
            });
            $("#demoPres").on("click",function(ev){
                // Oculto bloques
                let items=$(".fitem");
                if(items.hasClass("d-none")){
                    $(".fitem").removeClass("d-none");
                } else {
                    $(".fitem").addClass("d-none");
                }
            });
